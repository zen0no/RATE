...
# python3 TMaze_new/TMaze_new_src/train_rmdt_new_gtrxl.py --model_mode 'DT' --max_n_final 1 --end_seg 10 --arch_mode 'TrXL' --curr 'false'
...
parser.add_argument('--curr', type=str, default='true', help='Description of model_name argument')
...
for RUN in range(start_seg, end_seg+1):
    ...
    TEXT_DESCRIPTION = "no_es_rew_4_vs_min1"
    ...
    TMaze_data_generator(max_segments=config["max_segments"], multiplier=config["multiplier"], 
                         hint_steps=config["hint_steps"], desired_reward=config["desired_reward"], win_only=config["win_only"])
    ...
    prev_ep = None
    ######################################################
    if config["curriculum"].lower() == 'true':
        ...
        for n_final in range(min_n_final, max_n_final+1):
            ...
            combined_dataloader = CombinedDataLoader(n_init=min_n_final, n_final=config["sections"], multiplier=config["multiplier"], 
                                                     hint_steps=config["hint_steps"], batch_size=config["batch_size"], mode="", 
                                                     cut_dataset=config["cut_dataset"], desired_reward=config["desired_reward"], win_only=config["win_only"])
    ...
    elif config["curriculum"].lower() == 'false':
        print("MODE: CLASSIC")
        config["sections"] = max_n_final
        combined_dataloader = CombinedDataLoader(n_init=min_n_final, n_final=config["sections"], multiplier=config["multiplier"], 
                                                 hint_steps=config["hint_steps"], batch_size=config["batch_size"], mode="", 
                                                 cut_dataset=config["cut_dataset"], one_mixed_dataset=True, desired_reward=config["desired_reward"], win_only=config["win_only"])
        # Split dataset into train and validation sets
        full_dataset = combined_dataloader.dataset
        train_size = int(0.8 * len(full_dataset))
        val_size = len(full_dataset) - train_size
