...
def train(ckpt_path, config):
    ...
    for epoch in pbar:
        ...
        #                 memory = res[0][2:]
        #                 logits, val_loss = res[0][0], res[0][1]
        #                 mem_tokens = res[1]
        #                 #val_imgs.append(model.attn_map)

        #                 if wwandb:
        #                     wandb.log({"val_loss":  val_loss.item(), "learning_rate": lr})
                    # if model.flag == 1:
                    #     pbar.set_description(f"ep {epoch+1} it {it} tTotal {train_loss.item():.2f} vTotal {val_loss.item():.2f} lr {lr:e}")


        # # Early stoppingcommit=True
        ...
        if (epoch + 1) % 100 == 0 or epoch == config["epochs"] - 1:
            ...
            torch.save(model.state_dict(), ckpt_path + '_' + str(20) + '_KTD.pth')
...
for RUN in range(1, 3+1):
    ...
    TEXT_DESCRIPTION = "like_atari_nmt_15"##_bigger"#"loss_all"
    mini_text = "wo_erl_stop"#"no_curr_data_1_9_inf_on_9_segments"
    ...
    ckpt_path = f'../{current_folder}/VizDoom/VizDoom_checkpoints/fixed/{name}/'
    isExist = os.path.exists(ckpt_path)
    if not isExist:
        os.makedirs(ckpt_path)

    if config["wwandb"]:
        #run = wandb.init(entity="RATE", project="VizDoom_clear", name=name, group=group, config=config, save_code=True, reinit=True)
        run = wandb.init(project="VizDoom_fixed", name=name, group=group, config=config, save_code=True, reinit=True)
